...
def get_bbox(entity):
    ...
    # Para HATCH, ezdxf.bbox.extents() frequentemente retorna (0,0).
    # Extraímos os vértices manualmente dos boundary paths.
    ...
    try:
        ...
        if ext:
            mn, mx = ext[0], ext[1]
            # Verifica se o bbox é válido (não é zero)
            if abs(mx[0] - mn[0]) > 0.0001 or abs(mx[1] - mn[1]) > 0.0001:
                return (mn[:2], mx[:2])
    except Exception:
        pass

    # Fallback para LWPOLYLINE
    if entity.dxftype() == 'LWPOLYLINE':
        pts = [pt[:2] for pt in entity.get_points()]
        if pts:
...
def get_centro(entity):
    ...
    # Para HATCH, calcula diretamente dos vértices (mais confiável)
    if entity.dxftype() == 'HATCH':
        pts = _extrair_vertices_hatch(entity)
        if pts:
            xs = [p[0] for p in pts]
    ...
    for entity in msp:
        ...
        if entity.dxftype() == 'LWPOLYLINE':
            ...
            if abs(area - TRAFO_AREA) <= TOL_AREA:
                ...
                bbox = get_bbox(entity)
                ang = get_angulo(entity)
                lista_quad.append({"entity": entity, "centro": centro, "bbox": bbox, "ang": ang})
        elif entity.dxftype() == 'HATCH':
            area = get_area(entity)
            if abs(area - TRAFO_AREA) <= TOL_AREA:
                centro = get_centro(entity)
